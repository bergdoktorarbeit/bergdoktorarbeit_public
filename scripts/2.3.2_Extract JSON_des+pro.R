...
cat("Start Script 2.3.2_Extract JSON_des+pro.R:", format(start_time_2, "%H:%M:%S"), "\n")
...
# Set Winput path
download_ordner <- "2021-09-17_Instaloader_Download"
...
df_jpg_files_raw <- as.data.frame(jpg_files) %>% 
  ...
  mutate(jpg_id = str_extract(jpg_filepath, "(?<=Download/).+")) %>%
  mutate(jpg_id = gsub("_n_.jpg|_n.jpg", "", jpg_id)) %>% 
  mutate(image_id = str_extract(jpg_id, "(?<=_)\\d+_\\d+_\\d+"),
         acc = str_extract(jpg_id, "[\\w@\\-\\.+_]+(?=_\\d+_\\d+_\\d+)"),
         post_id = str_extract(jpg_id, "(?<=n_)[[:graph:]]+(?=.jpg)")) %>%
  select(jpg_filepath,
         jpg_id,
         image_id,
         post_id,
...
df_json_files_raw <- as.data.frame(json_files) %>% 
  ...
  mutate(json_id = str_extract(json_filepath, "(?<=Download/).+")) %>%
  mutate(json_id = gsub("_n_.json|_n.json", "", json_id)) %>% 
  mutate(image_id = str_extract(json_id, "(?<=_)\\d+_\\d+_\\d+"),
         acc = str_extract(json_id, "[\\w@\\-\\.+_]+(?=_\\d+_\\d+_\\d+)"),
         post_id = str_extract(json_id, "(?<=n_)[[:graph:]]+(?=.json)")) %>%
...
df_jpg_files_raw_des_pro <- df_jpg_files_raw
df_json_files_raw_des_pro <- df_json_files_raw
df_json_post_des_pro <- df_json_post
...
#save as csv 
  #NOTE: This script is called in 2.3.3 and is binded there with the 
...
cat("End Script:", format(Sys.time(), "%H:%M:%S"), "\n", 
    "Differenz: ", difftime(end_time_2, start_time_2, units = "min"), "min.", "\n") 
